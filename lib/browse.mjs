...
//
// The finder may only ever surface *directories* and *docs* (HTML or Markdown),
// and only within `root` (the $HOME boundary by default) — so a local page can't
// drive it to read arbitrary files. Every path is resolved and bounds-checked;
// `.git` and other dotfiles are hidden. This is the one security-critical module.
...
export async function browse(root, requested) {
  ...
  for (const name of names) {
    ...
    if (est.isDirectory()) {
      ...
      docs.push({ name, type: 'doc', path: full, kind: /\.(md|markdown)$/i.test(name) ? 'md' : 'html' });
    }
  }
...
// Validate a path for serving as a doc: an existing .html/.htm/.md/.markdown
// file under root. Returns the resolved absolute path, or throws a coded error.
export async function resolveDoc(root, requested) {
  if (!requested) throw fail('ENODOC', 'no doc specified');
  if (!DOC_RE.test(requested)) throw fail('ENOTDOC', 'not an .html/.htm/.md doc');
