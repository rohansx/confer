...
// Does a transcript for this session id exist in the workspace's project dir?
// Used to validate connect-by-id before binding, so a typo'd id fails at
// connect time instead of on the first question.
export async function sessionExists(workspace, id) {
  if (!ID_RE.test(id)) return false;
  const dir = await resolveProjectDir(workspace);
  return dir ? existsSync(path.join(dir, `${id}.jsonl`)) : false;
}

// Locate a session id across ALL project dirs and return the workspace (cwd)
// it was created in, or null. `claude --resume` only finds a session when run
// from that cwd, so connecting a doc to a session from another repo means
// spawning the agent there (with the doc's workspace added via --add-dir).
export async function findSessionHome(id) {
  if (!ID_RE.test(id) || !existsSync(PROJECTS)) return null;
  for (const d of await readdir(PROJECTS)) {
    const file = path.join(PROJECTS, d, `${id}.jsonl`);
    if (!existsSync(file)) continue;
    return firstCwd(await readHead(file, 4096));
  }
  return null;
}
...
// The id of the most recent Claude Code session for a workspace (for auto-connect),
// or null if none exist. Cheap: only stats files, no transcript parsing.
export async function latestSession(workspace) {
  const dir = await resolveProjectDir(workspace);
  if (!dir) return null;
  const files = (await readdir(dir)).filter((f) => UUID_RE.test(f));
  let best = null;
  for (const f of files) {
    try {
      ...
      if (!best || st.mtimeMs > best.mtime) best = { id: f.replace(/\.jsonl$/, ''), mtime: st.mtimeMs };
    } catch {}
  }
  return best?.id || null;
}

// Map of resolved workspace cwd -> session count, scanned once (one head read per
// project dir). Lets the file finder badge git-root folders without per-entry cost.
export async function sessionCounts() {
  ...
  for (const d of await readdir(PROJECTS)) {
    ...
    try {
      ...
      const head = await readHead(path.join(dir, files[0]), 4096);
      const cwd = firstCwd(head);
      if (cwd) out.set(path.resolve(cwd), files.length);
    } catch {}
  }
  return out;
}
